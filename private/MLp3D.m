function [paramsf, conflim] = MLp3D(data, params_start, numbins,verbose)
...
if verbose
  ...
  plot([pts xbin], 50/numbins*Q, 'r-');
  xlabel('Distance measurements')
  ylabel('Frequency')
  title('Histogram of data set with the fit to \itp_3_D(r)')
  legend('Data', 'ML fit to \itp_3_D(r)')
  hold off
end
...
if verbose
  % Display the results
  disp('Results of the maximum likelihood fit to p3D(r) from Eq.(4):');
