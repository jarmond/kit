...
%
% Created by:  Chris Smith 11/2014
...
rgbCrpd= zeros([2*ceil(opts.zoomScale*(2/pixelSize(1)))+1 ...
                ...
                3]);
rgbImgCS = zeros([job.cropSize(dims)*opts.chrShiftResolution, 3]);
rgbCrpdCS= zeros([opts.chrShiftResolution*2*ceil(opts.zoomScale*(2/pixelSize(1)))+1 ...
                opts.chrShiftResolution*2*ceil(opts.zoomScale*(2/pixelSize(1)))+1 ...
                3]);
...
for c = plotChans
    if c ~= coordSysChan && first
        ...
        [img1,img2,tempRem] = ... 
            ...
            'interpol',opts.chrShiftResolution);
    ...
    elseif c ~= coordSysChan
        [~,img2,tempRem] = ...
            kitChrShiftImage(rgbImg(:,:,mapChans(coordSysChan)),rgbImg(:,:,mapChans(c)),job.options.chrShift{coordSysChan,c}, ...
            'interpol',opts.chrShiftResolution);
...
if opts.zoomTrack
    ...
    for c = plotChans
        ...
        xRegCS(:,c) = xReg(:,c)*opts.chrShiftResolution - removed(1,1,c);
        yRegCS(:,c) = yReg(:,c)*opts.chrShiftResolution - removed(2,1,c);
...
coordCS = coord*opts.chrShiftResolution;
for c = 1:nChans
    ...
    if opts.zoomTrack
        ...
        coordCS(c,1,:) = coordCS(c,1,:) - (xRegCS(1,plotChans(c))+1) + chrShift(1)*opts.chrShiftResolution;
        coordCS(c,2,:) = coordCS(c,2,:) - (yRegCS(1,plotChans(c))+1) + chrShift(2)*opts.chrShiftResolution;
    else
        coord(c,1,:) = coord(c,1,:) + chrShift(1);
        coord(c,2,:) = coord(c,2,:) + chrShift(2);
        coordCS(c,1,:) = coordCS(c,1,:) + chrShift(1)*opts.chrShiftResolution;
        coordCS(c,2,:) = coordCS(c,2,:) + chrShift(2)*opts.chrShiftResolution;
    end
